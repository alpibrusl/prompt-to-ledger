...
- [ ] The forecast was computed with an actual spreadsheet or script, formulas visible — not narrated as a number (Chapter 4).
- [ ] The model is built with real parameters — growth rate, hiring plan, pricing — not a one-off sheet quietly tuned to one pitch (Chapter 5).
- [ ] Standard definitions were used for margin, CAC, LTV, and burn — not formulas invented for this spreadsheet (Chapter 6).
- [ ] The model was tested against a case with a known answer — last year's real numbers — and reproduced it, before being trusted on the year that matters (Chapter 7).

### Not fooling yourself

- [ ] The forecast passed a consistency check against something else already known — the bank statement, the pipeline, a board member's sense of the business — not only checked because it looked unusual (Chapter 7).
- [ ] An unusually good or unusually bad forecast specifically was checked for an assumption or formula error before it was believed (Chapter 7).
- [ ] The load-bearing assumption has been identified, and it's one you can defend with a reason, not just a hope (Chapter 8).
- [ ] Every already-committed hire, contract, or cost is reflected in the model, not just the current snapshot (Chapter 9).
- [ ] You have a range — base, worse, better — not just a single headline number, and it's narrow enough to actually plan against (Chapter 10).

### Knowing what you're looking at

- [ ] The underlying bookkeeping has been reconciled against the bank statement for the period the model covers (Chapter 13).
- [ ] Personal or miscategorised expenses have been checked for and cleaned out of the numbers feeding the model (Chapter 13).
- [ ] A chart's axis, date range, and choice of what to include would tell the same story drawn the plain, unflattering way (Chapter 12).

### If money is changing hands

- [ ] You understand what any fundraise or equity grant actually does to your ownership, including any option pool, before signing (Chapter 11).
- [ ] Anything with real legal, tax, or regulatory exposure has been reviewed by an actual professional, not settled from a model alone (Chapters 1 and 11).
...
4. **Do you have a range, not just a number, and does it include every commitment you've already made?**
5. **Has anything with real legal or tax exposure gone to an actual professional?**
