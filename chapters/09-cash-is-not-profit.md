...
Ledgerly's agent reported eighteen. The gap between twelve and eighteen was the same optimistic growth assumption Chapter 8 already caught: the model expected revenue to climb steeply enough, steeply enough soon, that burn would shrink month over month and the cash would stretch further before running out. Run the same runway calculation with Chapter 8's more defensible growth rate instead, and the eighteen-month figure collapses back down — most of the way toward the naive twelve.
...
One more piece of the cash picture rarely shows up until it does. **Working capital** is the cash tied up in the timing gap between earning revenue and actually collecting it, or between incurring a cost and actually paying it. A business can be profitable on the income statement and still watch its cash shrink, if its customers are slow to pay while its own bills come due on schedule — exactly Chapter 2's Ledgerly example, generalised into an ongoing pattern rather than one confusing month.

Growth makes this worse before it makes anything better. A business adding new customers every month is, by definition, extending more credit every month than it's collecting from the customers it signed further back — even if every single customer eventually pays in full. A model that only asks "will we be profitable" and never asks "will collections keep pace with growth" can miss a cash shortage that a profitable, healthy-looking business is genuinely walking toward.
