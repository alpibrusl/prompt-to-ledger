...
checks a financial model against this book's closing checklist before anyone hands you a forecast as though it were trustworthy.
...
"is this forecast sound?" and having something ask it for you every time.
...
The skill lives in this book's repository, at `skills/verify-ledger/`. Download
...
```
~/.claude/skills/verify-ledger/
```

That makes it available in every project. To scope it to one project
instead, copy it to `.claude/skills/verify-ledger/` inside that project.
...
```
~/.config/opencode/skills/verify-ledger/
```
